...
necessary_height = Z0 + Z_travel + x_end_height() + bar_clamp_depth + axis_endstop_clearance + base_clearance;
height = max(custom_height, necessary_height);
...
necessary_base_width = base_clearance - x_idler_overhang() + Z_bar_spacing -x_motor_overhang() + base_clearance;
base_width = max(custom_base_width, necessary_base_width);
...
idler_end = -necessary_base_width / 2 + base_clearance - x_idler_overhang();
motor_end =  necessary_base_width / 2 - base_clearance + x_motor_overhang();
X_origin = (idler_end + motor_end) / 2 + nozzle_x_offset;
left_w  = ceil((necessary_base_width - window_width)/2 + X_origin);
right_w = ceil((necessary_base_width - window_width)/2 - X_origin);
